    ...
    def get_embedding(self, text: str) -> List[float]:
        ...
        try:
            ...
            if isinstance(text, list):
                ...
                logger.info(f"Generated single embedding with {len(embeddings)}  dimensions")
                return embeddings   
        except Exception as e:
    ...
    def add_documents_to_collection(self, documents: List[Tuple[str, Dict[str, Any]]], 
        ...
        for batch_start in range(0,len(documents), batch_size):
            ...
            for text, metadata in batch:
                ...
                # Collect text and metadata for embedding
                texts_needing_embeddings.append(text)
                text_metadata_pairs.append((doc_id, text, metadata, exists))

            if texts_needing_embeddings:
                # Get embedding for the text
                try:
                    batch_embeddings = self.get_embedding(texts_needing_embeddings)
                except Exception as e:
                    logger.error(f"Failed to generate batch embeddings: {e}")
                    continue
                for (doc_id, text, metadata, exists), embedding in zip(text_metadata_pairs,batch_embeddings):
